...
CREATE TABLE IF NOT EXISTS private.ai_model_setting_prices (
    pricing_id uuid PRIMARY KEY DEFAULT gen_random_uuid(),
    model_id varchar(64) NOT NULL REFERENCES private.ai_model_catalog(model_id) ON DELETE CASCADE,
    config_key varchar(128) NOT NULL,
    settings jsonb NOT NULL DEFAULT '{}'::jsonb CHECK (jsonb_typeof(settings) = 'object'),
    tst_cost bigint CHECK (tst_cost IS NULL OR tst_cost BETWEEN 0 AND 1000000),
...
ALTER TABLE private.ai_model_setting_prices ENABLE ROW LEVEL SECURITY;
ALTER TABLE private.ai_model_setting_prices FORCE ROW LEVEL SECURITY;
REVOKE ALL ON private.ai_model_setting_prices FROM PUBLIC, anon, authenticated;
GRANT SELECT, INSERT, UPDATE ON private.ai_model_setting_prices TO service_role;
...
BEGIN
    ...
    IF action = 'sync' THEN
        ...
        FOR item IN SELECT value FROM jsonb_array_elements(payload->'models') LOOP
            ...
            IF jsonb_typeof(item->'pricing') = 'array' THEN
                FOR price_item IN SELECT value FROM jsonb_array_elements(item->'pricing') LOOP
                    config_key_value := left(md5(price_item::text), 128);
                    settings_value := price_item - ARRAY['credits', 'cost', 'price', 'key', 'config_key'];
                    provider_cost := CASE
                        WHEN COALESCE(price_item->>'credits', price_item->>'cost', price_item->>'price') ~ '^[0-9]+$'
                        THEN (COALESCE(price_item->>'credits', price_item->>'cost', price_item->>'price'))::bigint
                        ELSE NULL END;
        ...
        RETURN COALESCE((SELECT jsonb_agg(jsonb_build_object(
            ...
            FROM private.ai_model_catalog
            WHERE provider_metadata->>'type' LIKE '%image%'), '[]'::jsonb);
    END IF;

    IF action = 'quote' THEN
        model_id_value := lower(btrim(payload->>'modelId'));
        RETURN COALESCE((SELECT jsonb_build_object('creditCost', credit_cost,
            'pricingVersion', pricing_version, 'pricingId', pricing_id)
            FROM private.ai_model_setting_prices
            WHERE model_id = model_id_value AND active
              AND settings @> COALESCE(payload->'settings', '{}'::jsonb)
            ORDER BY jsonb_object_length(settings) DESC, credit_cost ASC LIMIT 1),
            (SELECT jsonb_build_object('creditCost', credit_cost, 'pricingVersion', pricing_version,
                'pricingId', pricing_id) FROM private.ai_model_setting_prices
                WHERE model_id = model_id_value AND active AND jsonb_object_length(settings) = 0
    ...
    IF action IN ('public', 'admin_list', 'admin_save') THEN
        ...
        IF action = 'admin_save' THEN
            ...
            IF NULLIF(payload->>'pricingId', '')::uuid IS NULL OR cost_value IS NULL
                OR cost_value NOT BETWEEN 1 AND 1000000 THEN
                RAISE EXCEPTION 'AI_MODEL_CATALOG_INVALID' USING ERRCODE = 'P0001';
            END IF;
            UPDATE private.ai_model_setting_prices SET credit_cost = cost_value,
                ...
                WHERE pricing_id = (payload->>'pricingId')::uuid RETURNING * INTO price_row;
            ...
            VALUES (admin_id, 'AI_MODEL_SETTING_PRICING_CHANGED', 'AI_MODEL_PRICING', price_row.pricing_id,
                ...
                jsonb_build_object('modelId', price_row.model_id, 'creditCost', price_row.credit_cost,
                    'active', price_row.active, 'reason', left(payload->>'reason', 500)));
            RETURN jsonb_build_object('pricingId', price_row.pricing_id, 'pricingVersion', price_row.pricing_version);
        END IF;
        RETURN COALESCE((SELECT jsonb_agg(jsonb_build_object(
            'pricingId', p.pricing_id, 'modelId', p.model_id, 'modelName', m.display_name,
            'settings', p.settings, 'tstCost', p.tst_cost, 'creditCost', p.credit_cost,
            'active', p.active, 'pricingVersion', p.pricing_version)
            ORDER BY m.display_name, p.model_id, p.settings::text)
            FROM private.ai_model_setting_prices p
            JOIN private.ai_model_catalog m ON m.model_id = p.model_id
