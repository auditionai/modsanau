...
-- Cập nhật function gift_code_redeem để kiểm tra redemptions_per_user
...
BEGIN
    ...
    -- Check how many times this device_profile_id has redeemed this gift code
    SELECT COUNT(*) INTO user_redemption_count
    FROM private.gift_code_redemptions
    WHERE gift_code_id = gift.gift_code_id
    AND device_profile_id = profile.device_profile_id;

    IF user_redemption_count >= gift.redemptions_per_user THEN
        RAISE EXCEPTION 'Gift code usage limit reached for this account.'
            USING ERRCODE='P0001', CONSTRAINT='GIFT_CODE_USER_LIMIT';
    END IF;
