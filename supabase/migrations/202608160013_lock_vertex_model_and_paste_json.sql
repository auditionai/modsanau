...
-- Vertex prompt composition is server-managed. The admin only supplies the
-- service-account JSON; project, region and model are derived/locked here.
...
DECLARE
    ...
    region_value constant text := 'us-central1';
    model_value constant text := 'gemini-3.6-flash';
BEGIN
    ...
    IF action = 'status' THEN
        RETURN COALESCE((SELECT jsonb_build_object(
            'configured', true, 'projectId', project_id, 'region', region,
            'modelId', model_id, 'modelPolicy', 'Gemini 3.6 -> Gemini 3.1',
            'updatedAt', updated_at)
            FROM private.ai_provider_config WHERE provider_id = 'vertex_ai'),
            jsonb_build_object('configured', false, 'modelPolicy', 'Gemini 3.6 -> Gemini 3.1'));
    END IF;
    ...
    IF credentials_text IS NULL OR length(credentials_text) NOT BETWEEN 200 AND 20000 THEN
        RAISE EXCEPTION 'VERTEX_CREDENTIALS_INVALID';
    END IF;
    BEGIN
        credentials := credentials_text::jsonb;
    ...
    VALUES ('vertex_ai', secret_id, project_value, region_value, model_value, actor_id)
    ...
    PERFORM private.admin_audit_record(actor_id, 'VERTEX_AI_CREDENTIAL_ROTATED', 'AI_PROVIDER', secret_id,
        correlation_id, jsonb_build_object('provider', 'vertex_ai', 'projectId', project_value,
            'modelPolicy', 'Gemini 3.6 -> Gemini 3.1'));
    RETURN jsonb_build_object('code', 'VERTEX_AI_CREDENTIAL_SAVED', 'configured', true,
        'projectId', project_value, 'region', region_value, 'modelId', model_value,
        'modelPolicy', 'Gemini 3.6 -> Gemini 3.1');
END
$function$;
