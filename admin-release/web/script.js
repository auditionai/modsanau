...
const views = {
  ...
  "ai-presets": ["MẪU TẠO ẢNH", "Quản lý mẫu tạo ảnh AI"],
};
const labels = {
  active: "Hoạt động",
  blocked: "Đã khóa",
...
async function paymentApi(route, options = {}, retry = true) {
  ...
  if (!response.ok)
    throw new Error(result?.error || "PAYMENT_ADMIN_REQUEST_FAILED");
  return result;
}

async function aiPresetApi(action, payload = {}) {
  const config = await ensureConfig();
...
async function loadAiPresets() {
  const presets = await aiPresetApi("preset_list");
  state.aiPresets = Array.isArray(presets) ? presets : [];
  $("[data-ai-presets-grid]").innerHTML = state.aiPresets.length ? state.aiPresets.map((item) =>
    `<article class="package-card ${item.active ? "" : "is-archived"}"><h3>${escapeHtml(item.name)}</h3><p>${item.active ? "Hoạt động" : "Đã tắt"}</p><div class="package-meta"><span>#${number(item.sortOrder)}</span></div><div class="package-actions operator-only"><button class="secondary-button" data-edit-ai-preset="${escapeHtml(item.presetId)}">Chỉnh sửa</button><button class="ghost-button" data-delete-ai-preset="${escapeHtml(item.presetId)}">Xóa</button></div></article>`).join("") : '<div class="empty-state">Chưa có mẫu tạo ảnh.</div>';
  applyRoleVisibility();
}

function openAiPreset(id) {
  const item = state.aiPresets.find((preset) => preset.presetId === id);
  const form = $("[data-ai-preset-form]"); form.reset(); form.active.checked = true; form.sortOrder.value = "0";
  if (item) { form.presetId.value = item.presetId; form.name.value = item.name; form.basePrompt.value = item.basePrompt; form.active.checked = item.active; form.sortOrder.value = item.sortOrder; }
  $("[data-ai-preset-title]").textContent = item ? "Chỉnh sửa mẫu tạo ảnh" : "Tạo mẫu tạo ảnh";
  $("[data-ai-preset-dialog]").showModal();
}
...
async function loadView(view) {
  ...
  try {
    ...
    if (view === "revenue") await loadAnalytics();
    ...
    if (view === "ai-presets") await loadAiPresets();
...
function renderAnalytics() {
  ...
  $("[data-device-legend]").innerHTML = [
    ["#36d399", "Active", a.activeDevices],
    ["#fbbf24", "Blocked", a.blockedDevices],
    ["#fb7185", "Revoked", a.revokedDevices],
  ]
    .map(
      (x) =>
...
async function loadPackages() {
  const data = await api("/v1/admin/packages");
  state.packages = data.items || [];
  $("[data-packages-grid]").innerHTML = state.packages.length
    ? state.packages
        .map(
          (p) =>
            `<article class="package-card ${p.archivedAt ? "is-archived" : ""}">${badge(p.isActive ? "active" : "disabled")}<h3>${escapeHtml(p.displayName)}</h3><p>${escapeHtml(p.description || "Chưa có mô tả")}</p><div class="package-price"><strong>${money(p.amountMinor, p.currency)}</strong><span>/ ${number(p.credits)} credits</span></div><div class="package-meta"><span class="mono">${escapeHtml(p.productId)}</span><span>#${p.sortOrder}</span></div><div class="package-actions operator-only"><button class="secondary-button" data-edit-package="${p.packageId}">Chỉnh sửa</button>${p.isActive ? `<button class="ghost-button" data-archive-package="${p.packageId}">Archive</button>` : ""}</div></article>`,
        )
        .join("")
        ...
        .map((g) => {
          const status = g.disabledAt
            ...
            : g.expiresAt && new Date(g.expiresAt) < new Date()
              ? "expired"
              : "active";
          return `<tr><td><strong class="mono">${escapeHtml(g.codePrefix)}…</strong><small class="mono">${escapeHtml(g.giftCodeId)}</small></td><td>${escapeHtml(labels[g.kind] || g.kind)}</td><td>${g.kind === "credits" ? number(g.creditAmount) + " credits" : number(g.durationDays) + " ngày"}</td><td>${number(g.redemptionCount)} / ${number(g.maximumRedemptions)}</td><td>${date(g.expiresAt)}</td><td>${badge(status)}</td><td><div class="row-actions operator-only">${status === "active" ? `<button data-revoke-gift="${g.giftCodeId}">Thu hồi</button>` : ""}</div></td></tr>`;
        })
        .join("")
...
document.addEventListener("click", async (e) => {
  ...
  if (b.matches("[data-new-ai-preset]")) openAiPreset();
  if (b.dataset.editAiPreset) openAiPreset(b.dataset.editAiPreset);
  if (b.dataset.deleteAiPreset) {
    const reason = await confirmAction("Xóa mẫu tạo ảnh", "Mẫu sẽ không còn được sử dụng cho yêu cầu mới.");
    if (reason !== null) { await aiPresetApi("preset_delete", { presetId: b.dataset.deleteAiPreset }); toast("Đã xóa mẫu tạo ảnh."); await loadAiPresets(); }
  }
...
$("[data-ai-preset-form]").addEventListener("submit", async (e) => {
  e.preventDefault(); const form = e.currentTarget; if (!form.reportValidity()) return;
  try {
    await aiPresetApi("preset_save", { presetId: form.presetId.value || null, name: form.name.value.trim(), basePrompt: form.basePrompt.value.trim(), active: form.active.checked, sortOrder: Number(form.sortOrder.value) });
    $("[data-ai-preset-dialog]").close(); toast("Đã lưu mẫu tạo ảnh."); await loadAiPresets();
  ...
  try {
    const result = await api("/v1/admin/gift-codes", {
      ...
      body: JSON.stringify({
        ...
        creditAmount: isCredits ? Number(f.creditAmount.value) : null,
...
$("[data-gift-kind]").addEventListener("change", (e) => {
  $("[data-credit-field]").hidden = e.target.value !== "credits";
  $("[data-duration-field]").hidden = e.target.value !== "duration";
});
