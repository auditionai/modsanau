...
Catalog, entitlement/nonce và private storage production adapters mặc định unavailable/fail-closed. PLAN 71 xác lập package/access contract và fake-storage gates; chưa wire raw download vào Create Project vì PLAN 72 phải quyết định encrypted local cache/key wrapping và atomic materialization. Khi adapter đó tồn tại, bytes phải được stream với bound/timeout/cancellation, đối chiếu exact length + trusted SHA-256 trước atomic publish; lỗi không được publish vào cache/workspace.
